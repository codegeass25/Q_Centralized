...
const DB_FILE = path.resolve(
  process.env.DB_FILE || path.join(__dirname, 'data', 'qlog-pro.sqlite3')const path = require('path');
const fs = require('fs');
const crypto = require('crypto');
const http = require('http');
const express = require('express');
...
async function initDb() {
  ...
  // Hardened legacy-schema migration: CREATE TABLE IF NOT EXISTS does not add
  // missing columns to an existing database. Ensure every column used by the
  // Central APIs exists BEFORE creating indexes or running card queries.
  ...
  for (const [table, columns] of Object.entries(requiredColumns)) {
    const existingCols = new Set((await all(`PRAGMA table_info(${table})`)).map(r => r.name));
  ...
  // Create indexes only AFTER the migration above. This prevents startup
  // SQLITE_ERROR such as "no such column" on legacy Central databases.
  await exec(`
